...
import queue
import cfscrape
from Scripts.parse_pakistanobserver import processArticleHtml
...
def getTodayUrls():
    ...
    while True:
        try:
            url = 'https://pakobserver.net/category/latest/'
            ...
            if status == 200:
                ...
                articles = soup.find_all('div', {'class', 'td-animation-stack'})
...
def worker():
    print('Pakistan Observer Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'PakistanObserver_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'pakistanObserver_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('Pakistan Observer: Article', url.split('/')[-2],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('Pakistan Observer: Error in processing')
