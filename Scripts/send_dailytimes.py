...
import queue
import cfscrape
from Scripts.parse_dailytimes import processArticleHtml
    ...
    while True:
        try:
            ...
            if status == 200:
                ...
                containers = soup.find_all('section', {'class': 'featured-content'})[:2]
...
def worker():
    print('Daily Times Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'DailyTimes_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'dailytimes_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('Daily Times: Article', url.split('/')[-2],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('Daily Times: Error in processing')
