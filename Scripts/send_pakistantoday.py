...
from Scripts.parse_pakistantoday import processArticleHtml
import queue
import cfscrape
import datetime
...
def getTodayUrls():
    ...
    while True:
        try:
            ...
            url = 'https://www.pakistantoday.com.pk/' + date[0] + '/' + date[1] + '/' + date[2] + '/'
            ...
            if status == 200:
                ...
                container = soup.find('div', {'class': 'content-inner'})
...
def worker():
    print('Pakistan Today Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'PakistanToday_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'pakistantoday_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('Pakistan Today: Article', url.split('/')[-2],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('Pakistan Today: Error in processing')
