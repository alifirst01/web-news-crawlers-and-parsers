...
def getAuthor(soup):
    try:
        author = soup.find('div',{'class':'capital'}).text
...
def processArticleHtml(file, url, log_file):
    ...
    Html_Folder = os.path.join(Parent_DIR, 'Htmls', 'PakistanToday_HtmlFiles')
    Json_DIR = os.path.join(Parent_DIR, 'Jsons', 'PakistanToday_JsonFiles')
    ...
    while retry:
        try:
            ...
            retry -= 1
            if retry == 0:
                print("Pakistan Today: Error Processing Json File " + str(article_id))
