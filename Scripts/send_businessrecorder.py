...
import queue
import cfscrape
from Scripts.parse_businessrecorder import processArticleHtml
    ...
    while True:
        try:
            ...
            if status == 200:
                ...
                containers = soup.find_all('div', {'class': 'cover-inner'})
                for container in containers:
                    articles = container.find_all('li')
                    for article in articles:
                        urls.append(article.find('a')['href'])
                    urls.append(container.find('div', {'class': 'lay-one'}).find('a')['href'])
            break
        except:
            if retry == 0:
                break
...
def worker():
    print('Business Recorder Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'BusinessRecorder_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'businessRecorder_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('Business Recorder: Article', url.split('/')[-2],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('Business Recorder: Error in processing')
