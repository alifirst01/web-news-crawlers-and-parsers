...
def processArticleHtml(file, url, log_file):
    ...
    Html_Folder = os.path.join(Parent_DIR, 'Htmls', 'Herald_HtmlFiles')
    Json_DIR = os.path.join(Parent_DIR, 'Jsons', 'Herald_JsonFiles')
    ...
    while retry:
        try:
            ...
            retry -= 1
            if retry == 0:
                print("Herald: Error Processing Json File " + str(article_id))
