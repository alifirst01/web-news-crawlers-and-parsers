...
import queue
import cfscrape
from Scripts.parse_herald import processArticleHtml
...
def getTodayUrls():
    ...
    while True:
        try:
            url = 'https://herald.dawn.com/archive'
...
def worker():
    print('Herald Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'Herald_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'herald_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('Herald: Article', url.split('/')[-1],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('Herald: Error in processing')
