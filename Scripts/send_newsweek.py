...
import queue
import cfscrape
from Scripts.parse_newsweek import processArticleHtml
...
def getTodayUrls():
    ...
    while True:
        try:
            url = 'http://newsweekpakistan.com/category/latest-news/'
...
def worker():
    print('News Week Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'NewsWeek_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'newsweek_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('News Week: Article', url.split('/')[-2],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('News Week: Error in processing')
