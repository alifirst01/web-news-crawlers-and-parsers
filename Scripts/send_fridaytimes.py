...
import queue
import cfscrape
from Scripts.parse_fridaytimes import processArticleHtml
...
def getTodayUrls():
    ...
    page = 'http://www.thefridaytimes.com/tft/category/latest-issue/'
    page_urls = [page, page + 'page/2/', page + 'page/3/']
    ...
    for url in page_urls:
        ...
        while True:
            try:
                ...
                response = requests.get(url, timeout=60, headers=headers, proxies=proxies)
                status = response.status_code
                res_text = response.text
                if status == 200:
                    soup = BeautifulSoup(res_text, 'html.parser')
                    articles = soup.find_all('div', {'class', 'post_wrapper'})
...
def worker():
    print('Friday Times Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'FridayTimes_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'fridayTimes_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('Friday Times: Article', url.split('/')[-2],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('Friday Times: Error in processing')
