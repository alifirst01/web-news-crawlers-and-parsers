...
def getPublishedDate(soup):
    try:
        date = soup.find('div', {'class':'post_detail2'}).text.split('Issue: ')[1]
    except:
        date = "None"
    return date
...
def getAuthor(soup):
    try:
        author = soup.find('div', {'class': 'post_detail2'}).find('strong').text
    except:
        author = "None"
    return author
...
def processArticleHtml(file, url, log_file):
    ...
    Html_Folder = os.path.join(Parent_DIR, 'Htmls', 'FridayTimes_HtmlFiles')
    Json_DIR = os.path.join(Parent_DIR, 'Jsons', 'FridayTimes_JsonFiles')
    ...
    while retry:
        try:
            ...
            retry -= 1
            if retry == 0:
                print("Friday Times: Error Processing Json File " + str(article_id))
