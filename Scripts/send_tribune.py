...
import queue
import cfscrape
from Scripts.parse_tribune import processArticleHtml
    ...
    while True:
        try:
            ...
            if status == 200:
                ...
                container = soup.find('div',{'class':'primary'})
                articles = container.find_all('div', {'class':'story'})
...
def worker():
    print('Tribune Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'Tribune_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'tribune_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('Tribune: Article', url.split('/')[-2],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('Tribune: Error in processing')
