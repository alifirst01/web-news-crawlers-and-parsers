...
def getAuthor(soup):
    try:
        author = soup.find('div', {'class':'author'}).text.split('by')[1]
    except:
        author = "None"
    return author
...
def processArticleHtml(file, url, log_file):
    ...
    Html_Folder = os.path.join(Parent_DIR, 'Htmls', 'FrontierPost_HtmlFiles')
    Json_DIR = os.path.join(Parent_DIR, 'Jsons', 'FrontierPost_JsonFiles')
    ...
    while retry:
        try:
            ...
            retry -= 1
            if retry == 0:
                print("Frontier Post: Error Processing Json File " + str(article_id))
