...
import queue
import cfscrape
from Scripts.parse_nation import processArticleHtml
...
def getTodayUrls():
    ...
    while True:
        try:
            url = 'https://nation.com.pk/latest'
            ...
            if status == 200:
                ...
                blogs = container.find_all('div', {'class': 'itemblog'})
                for blog in blogs:
                    urls.append(blog.find('a')['href'])
            break
        except:
            if retry == 0:
                break
...
def worker():
    print('Nation Thread Starting')
    Parent_DIR = os.getcwd()
    DIR = os.path.join(Parent_DIR, 'Htmls', 'Nation_HtmlFiles')
    ...
    logs = open(os.path.join(LOG_DIR, 'nation_log.txt'), 'a')
        ...
        while retry >= 0:
            try:
                ...
                if status == 200:
                    ...
                    print('Nation: Article', url.split('/')[-1],'Saved')
                retry = -1

            except Exception as e:
                if retry == 0:
                    print('Nation: Error in processing')
